...
import { AvaContext } from './NewFileForm';
...
const LoadData = ({ width }: ILoadDataProps) => {
    ...
    const getImageContext = useContext(ImageContext); //контекст для других видов(файлов)
        ...
        ExifReader.load(file).then(data => {
            ...
            if (getImageContext) {
                getImageContext.updateMetaData({
                    date: data.DateTime ? data.DateTime.description : 'Нет даты',
                    latitude: data.GPSLatitude ? data.GPSLatitude.description : 'Нет широты',
                    longitude: data.GPSLongitude ? data.GPSLongitude.description : 'Нет долготы'
                })
            }
        })
    }
